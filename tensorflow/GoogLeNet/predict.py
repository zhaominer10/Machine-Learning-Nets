import os
import glob
import json

from PIL import Image
import numpy as np
import matplotlib.pyplot as plt

from model_bn import GoogLeNet


def main():
    im_height = 224
    im_width = 224

    # load image
    img_path = "../../dataset/predict_image/sunflower.jpg"
    assert os.path.exists(img_path), "file: '{}' dose not exist.".format(img_path)
    img = Image.open(img_path).convert('RGB')
    # resize image to 224x224
    img = img.resize((im_width, im_height))
    plt.imshow(img)

    # scaling pixel value and normalize
    img = ((np.array(img) / 255.) - 0.5) / 0.5

    # Add the image to a batch where it's the only member.
    img = (np.expand_dims(img, 0))

    # read class_indict
    json_path = './class_indices.json'
    assert os.path.exists(json_path), "file: '{}' dose not exist.".format(json_path)

    with open(json_path, "r") as f:
        class_indict = json.load(f)

    model = GoogLeNet(num_classes=5, aux_logits=False)
    # model.summary()

    # model.load_weights("./save_weights/myGoogLenet.h5", by_name=True)  # h5 format
    weights_path = "./weights/myGoogLeNet.ckpt"
    assert len(glob.glob(weights_path + "*")), "cannot find {}".format(weights_path)
    model.load_weights(weights_path)

    # # 模型在训练的时候，model的aux_logits参数为True，也就是包含了辅助分类器，但是在预测时，并没有加载辅助分类器（aux_logits=False）
    # # 因此通过h5加载模型时，需要设置by_name=True，表示通过模型变量名加载权重
    # # ckpt没有这个问题
    # weights_path_h5 = "./myGoogLeNet.h5"
    # assert len(glob.glob(weights_path_h5 + "*")), "cannot find {}".format(weights_path)
    # model.load_weights(weights_path_h5, by_name=True)

    result = np.squeeze(model.predict(img))
    predict_class = np.argmax(result)

    print_res = "class: {}   prob: {:.3}".format(class_indict[str(predict_class)],
                                                 result[predict_class])
    plt.title(print_res)
    for i in range(len(result)):
        print("class: {:10}   prob: {:.3}".format(class_indict[str(i)],
                                                  result[i]))
    plt.show()


if __name__ == "__main__":
    main()
